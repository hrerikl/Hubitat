 ...
 *    2019-1-29   Erik Lundby    Modified to Represent a Aeotec WallSwipe
 */

// for the UI
metadata {
	definition (name: "Nano Wall Swipe ZW158", namespace: "hrerikl", author: "Erik Lundby based on code by Allan (vseven) - based on code by Dan Ogorchock") {
	capability "Switch"		
...
def setColor(Map color) {
    ...
    if (color.hex){
        ...
        return adjustColor(colorHEX)
    }
}

def setLevel(value,duration=null) {
	//ignoring duration for now
...
def sendData(String value) {
	...
	if (logEnable) log.debug "calling Parent with ${name} ${value}"
    return getParent().sendData("${name} ${value}")  
}
...
def hueToRgb(v1, v2, vh) {
	log.debug("hueToRgb called with $v1, $v2, $vh")
	if (vh < 0) { vh += 1 }
	if (vh > 1) { vh -= 1 }
