 ...
 *  Date:  2019-01-29
 * 
 *  Based On custom device handler by ccheng@aeon-labs.com based on work by Eric Maycock (erocm123)
 *   
 *	github: Eric Maycock (erocm123) -> Converted to Nano Dimmer custom device handler ccheng@aeon-labs.com
...
def update_children (cmd)
{
	//if a wall swipe has been connected and no children exist create the child device
	if (cmd.parameterNumber==144)
	{
		if (cmd2Integer(cmd.configurationValue)==1 && getChildDevices().size()==0)
		{
			logging("Wall Swipe is Connected. Creating child device for it.");
			createChildDevices();
			
		}
		if (cmd2Integer(cmd.configurationValue)==0 && getChildDevices().size()>0)
		{
			logging("No Wall Swipe is Connected. Removing it's child device.");
			deleteChildDevices();
		}
		return;
	}
       ...
       //logging("Flag Placement")
            ...
            //logging("Flag Placement")
                //if (device.currentValue("currentFirmware") == null || "${it.@fw}".indexOf(device.currentValue("currentFirmware")) >= 0){
					...
					logging("Converted Configuration value = $convertedConfigurationValue")
				    logging("Configuration Value =${integer2Cmd(convertedConfigurationValue, it.@byteSize.toInteger())}")
...
/**
* Convert 1-4 bytes values to integer
*/
...
def zwaveEvent(hubitat.zwave.commands.configurationv1.ConfigurationReport cmd) {
	logging "Recieved Configurationv1 Report"
...
def zwaveEvent(hubitat.zwave.commands.configurationv2.ConfigurationReport cmd) {
	logging "Recieved Configurationv2 Report"
    logging("${device.displayName} parameter '${cmd.parameterNumber}' with a byte size of '${cmd.size}' is set to '${cmd2Integer(cmd.configurationValue)}'")
	update_current_properties(cmd)
	update_children(cmd)
}
...
{
	def cmds=[]
	logging("Child Wall Swipe Called $value")
	ledvalue=value.split()[-1].split('#')[-1]
	logging("Configuration Value =${integer2Cmd( Integer.parseInt(ledvalue,16),4)}")
	cmd=zwave.configurationV1.configurationSet(configurationValue:integer2Cmd( Integer.parseInt(ledvalue,16),4), parameterNumber: 64, size: 4)
	logging( "LED with a byte size of '${cmd.size}' is set to '${cmd2Integer(cmd.configurationValue)}'")
	cmds << cmd
	cmds << zwave.configurationV1.configurationGet(parameterNumber: 64)
	
	sendHubCommand(response(commands(cmds)))
...
private void createChildDevices() {
    logging( "createChildDevices Creating Child Device For Wall Swipe")
	addChildDevice("hrerikl", "Nano Wall Swipe ZW158", "$device.deviceNetworkId-Swipe", [name: "$device.name Wall Swipe", label: "$device.displayName Wall Swipe", isComponent: true])
}
